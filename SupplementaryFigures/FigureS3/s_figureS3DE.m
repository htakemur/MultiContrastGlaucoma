function s_figureS3DE
%This code aims to reproduce panels D and E of Supplementary Figure 3 in the following paper.
...
[r(1,2), p(1,2)] = corr(transpose(glaucoma_fiso_individual), transpose(visualfield));

% Plot ODI and visual field test
figure(2)
scatter(transpose(glaucoma_fiso_individual), transpose(visualfield),'Marker','o','MarkerEdgeColor','k', 'MarkerFaceColor','none');
xlim([-6 6]);
ylim([-30 10]);
xtick = [-6 0 6];
ytick = [-30 -20 -10 0 10];
...
Critical = 0.05;%critical region
y = visualfield';
x = [ones(size(y)) glaucoma_fiso_individual'];
b = regress(y,x);
yfit = x*b;
repetition = 10000;
[coefficient index] = bootstrp(...
...
xlabel('IsoV Residual (s.d. from control mean)');
ylabel('Visual field test');
axis square