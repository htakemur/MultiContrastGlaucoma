...
control_mean = mean(md_control_avg,2);
glaucoma_mean = mean(md_glaucoma_avg,2);
control_std = std(md_control_avg,0,2);
glaucoma_std = std(md_glaucoma_avg,0,2);

% Average data along nodes
control_mean_allnodes = mean(md_control_avg,1);
glaucoma_mean_allnodes = mean(md_glaucoma_avg,1);
...
for kk = 1:17
     plot([1:1:80],md_glaucoma_avg(:,kk),'color',[0 0 1],'LineWidth',0.5);
     hold on
end
